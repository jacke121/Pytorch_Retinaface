...
if __name__ == '__main__':
    ...
    img_raw = cv2.resize(img_raw, (300, 225))
    ...
    while True:  # 循环读取视频帧
        ...
        img = torch.from_numpy(np.asarray([img,img,img,img,img,img,img,img,img,img,img,img,img,img,img,img,img,img,img,img,img,img,img,img,img,img,img,img,img,img,img,img]))#.unsqueeze(0)
        img = img.to(device)


        locs, confs, landms = net(img)  # forward pass
        ...
        for i in range(locs.size(0)):
            ...
            landm = decode_landm(landm.data.squeeze(0), prior_data, cfg['variance'])
            ...
            landm = landm[:args.keep_top_k, :]

            dets = np.concatenate((dets, landm), axis=1)
        print('net forward time: {:.4f}'.format(time.time() - tic),locs.size())
            # show image
            # if args.save_image:
            #     for b in dets:
